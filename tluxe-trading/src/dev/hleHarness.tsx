/**
 * DEVELOPMENT HARNESS — visual verification of the High / Low Engine page with the
 * deterministic multi-timeframe TEST FIXTURE (aggregated from one M1 path, so all five
 * timeframes agree). Served only by the dev server at /hle-harness.html; not a production
...
class FixtureProvider implements MarketDataProvider {
  ...
  requestCandles(id: string, tf: Timeframe) {
    const c = HLE_TIMEFRAMES.includes(tf as never) ? (data[tf as keyof typeof data] ?? []) : [];
...
if (!location.hash) location.hash = '#/engines/high-low-engine';
