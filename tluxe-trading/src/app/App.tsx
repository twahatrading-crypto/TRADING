import { HLE_ROUTE, HLR_ROUTE, LIQUIDITY_ROUTE, MODULES, ORDER_BLOCKS_ROUTE, SETTINGS_ROUTE, SR_ROUTE } from '../config/modules';
...
export function App() {
  ...
  return (
    <AppShell>
      {route === SR_ROUTE ? <SRPage /> : route === LIQUIDITY_ROUTE ? <LiquidityPage /> : route === ORDER_BLOCKS_ROUTE ? <OrderBlocksPage /> : route === HLR_ROUTE ? <HLRPage /> : route === HLE_ROUTE ? <HighLowEnginePage /> : route === SETTINGS_ROUTE ? <SettingsPage /> : module ? <ModulePage module={module} /> : <Dashboard />}
    </AppShell>
  );
}
