/**
 * TEST FIXTURES ONLY. Deterministic synthetic candles for High / Low Reversal tests.
...
export function dataset(coarseH1: readonly Candle[], m1: readonly Candle[]): HLRInput {
...
export function mirrorInput(input: HLRInput, axis: number): HLRInput {
  const out: HLRInput = {};
  for (const [tf, c] of Object.entries(input) as [keyof HLRInput, Candle[]][]) out[tf] = mirrorCandles(c, axis);
  return out;
}
