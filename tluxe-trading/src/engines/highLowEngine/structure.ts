...
import type { HLEBreak, HLESwing } from './types';

type RtSwing = HLESwing & { index: number; confirmedIndex: number; broken: boolean };

/**
 * One timeframe of CLOSED candles for the High / Low Engine (its own code,
...
export class Series {
  ...
  push(bar: Candle): { swings: HLESwing[]; breaks: HLEBreak[] } {
  ...
  private detectBreak(i: number, dir: 'up' | 'down'): HLEBreak | null {
  ...
  private detectSwings(i: number): HLESwing[] {
    const L = this.left;
    const R = this.right;
    const c = i - R;
    if (c - L < 0) return [];
  ...
  unbroken(kind: 'high' | 'low'): HLESwing[] {
  ...
  lastSwings(kind: 'high' | 'low', n: number): HLESwing[] {
    const out: HLESwing[] = [];
