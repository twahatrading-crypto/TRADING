...
import type { HLRDrawable } from '../hlReversal/hlrView';
import { layoutLabels } from './labelLayout';

type Target = Parameters<IPrimitivePaneRenderer['draw']>[0];

/** BUY green/teal · SELL red · M5 structure blue · entry zone purple · selected H1 level gold. */
const TONE: Record<HLRDrawable['tone'], { rgb: string; text: string }> = {
  buy: { rgb: '60,201,160', text: '#86e3c6' },
  sell: { rgb: '239,93,93', text: '#f3a0a0' },
  structure: { rgb: '91,140,255', text: '#a9c1ff' },
  zone: { rgb: '167,139,250', text: '#cdbdfd' },
...
class Renderer implements IPrimitivePaneRenderer {
  constructor(
    private readonly src: HLRPrimitive,
...
/** Series primitive for High / Low Reversal overlays (engine output prepared by the UI). */
export class HLRPrimitive implements ISeriesPrimitive<Time> {
  ...
  setItems(items: HLRDrawable[]): void {
    this.items = items;
    this.requestUpdate?.();
  }
}
