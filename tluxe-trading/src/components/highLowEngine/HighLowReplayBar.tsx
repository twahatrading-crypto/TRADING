...
import { HLE_REPLAY_SPEEDS, type HighLowReplaySession } from '../../services/highLowEngine/HighLowReplay';
import { runHighLowAudit, type HLEAuditReport } from '../../services/highLowEngine/replayAudit';
import { useStore } from '../../store/createStore';
import { fmtUtc } from './useHighLow';

/** High / Low Engine replay transport (separate from every other replay). */
export function HighLowReplayBar({ session, onExit }: { session: HighLowReplaySession; onExit: () => void }) {
  const s = useStore(session.store, (x) => x);
  const [audit, setAudit] = useState<{ running: boolean; progress: string; report: HLEAuditReport | null }>({ running: false, progress: '', report: null });
  const atStart = s.cursor <= 0;
  const atEnd = s.cursor >= s.total - 1;
  const verify = async () => {
    session.pause();
    setAudit({ running: true, progress: '…', report: null });
    const report = await runHighLowAudit(session.dataset, s.timeframe, (label) => setAudit((a) => ({ ...a, progress: label })));
    setAudit({ running: false, progress: '', report });
  };
  const r = audit.report;
  return (
    <div className="replay hlereplay" role="region" aria-label="High / Low Engine replay controls" data-testid="hle-replay-bar">
      <div className="replay__row">
        ...
        <dl className="replay__read">
          <div><dt>Replay time</dt><dd data-testid="hle-replay-time">{fmtUtc(s.knowledgeTime)}</dd></div>
          <div><dt>Visible</dt><dd className="num" data-testid="hle-replay-count">{(s.cursor + 1).toLocaleString('en-US')} / {s.total.toLocaleString('en-US')}</dd></div>
          <div>
            <dt>Parity</dt>
            <dd className={s.parity ? (s.parity.ok ? 'ok' : 'bad') : ''} data-testid="hle-replay-parity" title="Incremental replay state vs a clean recomputation from the candles known at this time">
      ...
      {r && (
        <div className={`raudit ${r.passed ? 'is-pass' : 'is-fail'}`} data-testid="hle-audit">
          ...
          <table className="raudit__table">
            <thead>
              <tr><th>Steps</th><th>Levels</th><th>Setups</th><th>Sweeps</th><th>Reclaims</th><th>M5</th><th>Entry ready</th><th>Events</th><th>Result</th></tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td className="num">{r.audit.events}</td>
                <td className={r.audit.violations.length ? 'bad' : 'ok'}>{r.audit.violations.length ? `${r.audit.violations.length} issue(s)` : 'PASS'}</td>
              </tr>
              <tr>
                <td colSpan={8} className="srmuted">replay parity · {r.replay.timeframe} · {r.replay.steps} positions vs clean recomputation</td>
                <td className={r.replay.mismatches.length ? 'bad' : 'ok'}>{r.replay.mismatches.length ? `${r.replay.mismatches.length} issue(s)` : 'PASS'}</td>
              </tr>
            </tbody>
          </table>
          {r.firstMismatch && <p className="hle-firstmismatch" data-testid="hle-first-mismatch"><strong>First mismatch:</strong> {r.firstMismatch}</p>}
          {!r.passed && (
